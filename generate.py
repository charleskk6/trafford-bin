...
#gcal-config { margin-top:14px; padding-top:14px; border-top:1px solid var(--border);
  display:flex; flex-direction:column; gap:8px; }
#gcal-config label { font-size:.85rem; font-weight:600; display:flex; flex-direction:column; gap:6px; }
#gcal-config input { padding:9px 10px; border:1px solid var(--border); border-radius:8px;
  font-size:.9rem; width:100%; background:var(--card); color:var(--text); }
.hint { font-size:.78rem; color:var(--muted); margin:2px 0 0; }
.hint a { color:var(--purple); }
...
(function () {
  ...
  var GCAL_CLIENT_ID_DEFAULT = __GCAL_CLIENT_ID__;
  var GCAL_STORE = "trafford-gcal-client-id";
  ...
  function gcalClientId(){return ((localStorage.getItem(GCAL_STORE)||GCAL_CLIENT_ID_DEFAULT)||"").trim();}
  ...
  function getToken(){return new Promise(function(res,rej){
    var cid=gcalClientId();
    if(!cid){rej(new Error("Add your Google OAuth Client ID first — tap “Client ID”."));return;}
  ...
  var cfg=document.getElementById("gcal-config");
  document.getElementById("gcal-config-btn").addEventListener("click",function(){
    cfg.hidden=!cfg.hidden;
    if(!cfg.hidden){document.getElementById("gcal-client-id").value=(localStorage.getItem(GCAL_STORE)||"");}
  });
  document.getElementById("gcal-save").addEventListener("click",function(){
    var v=document.getElementById("gcal-client-id").value.trim();
    if(v){localStorage.setItem(GCAL_STORE,v);}else{localStorage.removeItem(GCAL_STORE);}
    cfg.hidden=true;gcalStatus(v?"Client ID saved. Tap “Sync to Google Calendar”.":"Client ID cleared.");
  });
...
{render_legend()}
    ...
    <section class="card gcal-card">
      ...
      <div class="gcal-actions">
        ...
        <button id="gcal-config-btn" class="btn btn-ghost btn-sm">Client ID</button>
      </div>
      <div id="gcal-config" hidden>
        <label for="gcal-client-id">Google OAuth Client ID
          <input id="gcal-client-id" type="text" autocomplete="off" spellcheck="false"
                 placeholder="1234567890-abc123.apps.googleusercontent.com" />
        </label>
        <div class="gcal-actions" style="margin-top:0">
          <button id="gcal-save" class="btn btn-sm">Save</button>
        </div>
        <p class="hint">Create a <strong>Web application</strong> OAuth client in Google Cloud Console, enable the Google Calendar API, and add this site's URL to the client's Authorized JavaScript origins. Stored only in your browser.</p>
      </div>
    </section>
  </main>
